...
def z14():
    ...
    nn=20
    maxx=0
    l3=map(lambda x,y:  1+12*x*y, l1,l2)#l3相当于是年利率
        ...
        for j in range(nn/l1[1]+1):
            for a in range(nn/l1[2]+1):
                for b in range(nn/l1[3]+1):
                    ...
                    if t>=0 :
                        kk=[i,j,a,b,t]
                        kt=reduce(lambda x, y: x*y, map(lambda x,y:  x**y,l3,kk))#reduce，map是什么用法？
                        if kt>maxx :
                            maxx=kt
                            kkk=kk
    print kkk,2000*maxx
